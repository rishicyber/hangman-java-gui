...
public class GameWindow extends JFrame implements ActionListener {
    ...
    String[] word = { "JAPAN", "QATAR", "SYRIA", "MONGOLIA", "BAHRAIN", "INDIA", "PAKISTAN", "AUSTRALIA", "AFGHANISTAN",
            "CHINA" };
    Random rd;
    StringBuffer blanks;
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        for (int loop = 0; loop < len; loop++) {
            if (letter.charAt(0) == word[rnd].charAt(loop)) {
                flag = 1;
                blanks.replace((loop), (loop + 1), letter);
                // blanks[loop]=letter.charAt(0)
                count++;
            }
        }
        if (flag == 1) {
